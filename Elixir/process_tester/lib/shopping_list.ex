defmodule ShoppingList do
  ...
  def add(pid, item) do
    GenServer.cast(pid, item) # sync -> we don't care about the response!
  end

  def view(pid) do
    GenServer.call(pid, :view) #async -> we care about the response!
  end

  def remove(pid, item) do
    GenServer.cast(pid, {:remove, item}) # sync...
  end
